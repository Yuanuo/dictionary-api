...
public class DictionaryTest {
    public static void main(String[] args) {
        ...
        // 列出默认的词典库列表
        Dictionaries.getDictionaries().list.forEach(System.out::println);

        // 根据词典ID获取词条实例
        Dictionary dictionary = Dictionaries.getDictionary("佛学辞典汇集");
        ...
        // 列出查询结果
        Dictionaries.getDictionary("佛学辞典汇集").search("三毒").forEachRemaining(entry -> {
            System.out.println(entry.title()); // 显示词条名称
            System.out.println(entry.contentText()); // 显示词条内容（原始）
        });
    }
